...
class CommandExecutor(ABC):
    ...
    @abstractmethod
    def run(self, cmd: List[str], check: bool = True, capture_output: bool = True, text: bool = True, background: bool = False) -> subprocess.CompletedProcess:
        """
        Execute a command.
...
class SshExecutor(CommandExecutor):
    ...
    def run(self, cmd: List[str], check: bool = True, capture_output: bool = True, text: bool = True, background: bool = False) -> subprocess.CompletedProcess:
        ...
        ssh_base.extend(["-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null"])
        ...
        return subprocess.run(ssh_base, check=check, capture_output=capture_output, text=text)
